...
class ReportConfig:
    ...
    metrics: List[str] = field(default_factory=lambda: ["BlendedCost", "UnblendedCost"])
...
class AWSCostReporter:
    ...
    def generate_report(self, config: ReportConfig) -> Any:
        """
        ...
        group_by_clause: List[Dict[str, str]] = []
        ...
        # Mypy: boto3-stubs can't verify **kwargs dict here; ignore argument type check
        response = self.ce_client.get_cost_and_usage(**params)  # type: ignore[arg-type]
        return response
    ...
    def _build_filter_clause(self, config: ReportConfig) -> Optional[Dict[str, Any]]:
        ...
        if config.linked_account_ids:
            filters.append(
                {
                    "Dimensions": {"Key": "LINKED_ACCOUNT", "Values": config.linked_account_ids}
                }
            )
        ...
        if config.tag_filters:
            for tag_key, tag_values in config.tag_filters.items():
                filters.append({"Tags": {"Key": tag_key, "Values": tag_values}})
    ...
    def _default_time_period(self) -> "DateIntervalTypeDef":
        """Return a default 30-day time period for CE helper endpoints."""
        ...
        for time_result in report_data["ResultsByTime"]:
            ...
            if not time_result.get("Groups"):
                ...
                for group in time_result["Groups"]:
                    base_row = {"StartDate": start_date, "EndDate": end_date}
    ...
    def get_linked_accounts(self) -> Dict[str, str]:
        ...
        response = self.ce_client.get_dimension_values(
            ...
            TimePeriod=self._default_time_period(),
        )
        return {
    ...
    def get_tag_values(self, tag_key: str) -> List[str]:
        """Get all possible values for a specific tag (via group-by TAG query)."""
        time_period: "DateIntervalTypeDef" = self._default_time_period()
        # Query with a TAG group-by to enumerate tag values
        result = self.ce_client.get_cost_and_usage(  # type: ignore[arg-type]
        ...
        for by_time in result.get("ResultsByTime", []):
            for group in by_time.get("Groups", []):
                for key in group.get("Keys", []):
                    # Keys are often in the format "tagKey$tagValue"; extract value portion
                    if "$" in key:
