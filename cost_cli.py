...
Available group by options:
  ...
  operation, purchase_type, platform, tenancy
    ...
    time_group.add_argument('--date-range', nargs=2, metavar=('START', 'END'),
                          help='Start and end dates (YYYY-MM-DD)')
    
    # Basic options
    parser.add_argument('--account', '--linked-account', 
                       help='Linked account ID')
    parser.add_argument('--accounts', '--linked-accounts', 
                       help='Comma-separated linked account IDs')
    parser.add_argument('--granularity', choices=['daily', 'monthly', 'hourly'],
                       default='monthly', help='Report granularity')
    parser.add_argument('--group-by', 
                       help='Comma-separated group by options')
    parser.add_argument('--service', 
                       help='Service shortname (rds, ec2, s3, lambda) or full name')
    parser.add_argument('--filters',
                       help='Filters in format: key1=val1,val2;key2=val3')
    
    # Output options
    parser.add_argument('--output', '-o', default='cost_report.csv',
